...
import 'package:insighteye_app/constants/pendingpgmdata.dart';
  ...
  void pendingupdate() async {
    ...
    if (_formKey.currentState!.validate()) {
      if (_value == true) {
        ...
        fb.collection("organizations")
          .doc("${widget.orgId}").collection("Programs").doc(widget.docname).delete().catchError(
            (error) =>
          ...
          showDialog(
              ...
              builder: (BuildContext context) {
                return CustomeAlertbx("Something went Wrong, Try again!",
                    Colors.redAccent, "Error", widget.techuid);
              });
        });
      } else {
        setState(() {
...
class CustomeAlertbx extends StatelessWidget {
  CustomeAlertbx(this.titles, this.colorr, this.done, this.techuid,
      {super.key});
